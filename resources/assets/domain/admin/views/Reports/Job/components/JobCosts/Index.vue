    ...
    <div class="form-container">
        ...
        <div class="content-wrapper">
            <div class="container-fluid col-v-5" style="background: white;">
                <h2 class="pageHeading"> Job Costs </h2>
                <template>
                ...
                >
                    ...
                    <span
                    slot="last_invoiced"
                    slot-scope="{ last_invoiced }"
                    >
                    {{ displayMoney(last_invoiced) }}
                    </span>
                    <span
                    slot="last_invoiced_date"
                    slot-scope="{ last_invoiced_date }"
                    >
                    {{ last_invoiced_date }}
                    </span>
                    <span
                    slot="creditors"
                    slot-scope="{ creditors }"
                    >
                    {{ displayMoney(creditors) }}
                    </span>
                    <span
                    slot="contractors"
                    slot-scope="{ contractors }"
                    >
                    {{ displayMoney(contractors) }}
                    </span>
                    <span
                    slot="total_cost"
                    slot-scope="{ total_cost }"
                    >
                    {{ displayMoney(total_cost) }}
                    </span>
                    <span
                    slot="margin_to_date"
                    slot-scope="{ margin_to_date }"
                    >
                    {{ displayMoney(margin_to_date) }}
                    </span>
                </fm-table-sortable>
                </div>
...
import { getSites, getJobCostsReport } from '../../../../../api/calls';
...
const TABLE_COLUMNS = [
    ...
    {
      id: 'last_invoiced_date',
      label: 'Last Invoiced Date',
    },
    {
      id: 'creditors',
      label: 'Creditors',
...
export default {
    ...
    methods: {
        ...
        exportReport(type) {
            window.location.href = `/api/reports/job/job-costs/export?${$.param({...this.query, ...this.filterData, 'type': type})}`;
        },
        setFilters(filters = {}) {
            const page = 1;
        ...
        fetchJobCosts(params) {
            this.enableLoading();
            getJobCostsReport({...DEFAULT_PARAMS, ...params, ...this.filterData})
            .then(({ data }) => {
                this.logs = data.data;
