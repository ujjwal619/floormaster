...
class UnlinkAllocation extends AdminController
{
    public function __invoke(Allocation $allocation, DatabaseManager $databaseManager)
    {
        try {
            return $databaseManager->transaction(function () use ($allocation, $databaseManager) {
                ...
                $allocation->fill(['is_linked' => false])->save();
                event(new CalculateStockTotal($color));

                $databaseManager->commit();

                return $this->sendSuccessResponse([], 'Successfully unlinked allocation.');
            });
        } catch (\Exception $exception) {
            $databaseManager->rollBack();
        }

        return $this->sendError('Could not unlink allocation.');
    }
}
